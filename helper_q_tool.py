...
def plot_acc_loss(q_history, x_history, v_history, data_ix):
    ...
    ax1.set_title("Qualcomm Speech Command Dataset Validation Accuracy")
    ax1.legend()
    ...
    ax2.set_title("Qualcomm Speech Command Dataset Validation Loss")
    ax2.legend()
    plt.tight_layout()
    plt.savefig("checkpoints/qualcom_checkpoints/astc" + data_ix +"_conv_speech_loss.png")

def plot_acc_loss2(q_history, data_ix):

    plt.figure()
    plt.style.use("seaborn")
    plt.plot(q_history.history["loss"], "-og", label="with Quanv Layer")
    #fig, (ax2) = plt.subplots(2, 1, figsize=(6, 9))

    #ax2.set_ylim(top=5.5)
    plt.xlabel("Epoch")
    plt.ylabel("Error Loss")
    plt.title("Qualcomm Speech Command Dataset Quantum Error Loss")
    plt.tight_layout()
    plt.legend()
    plt.savefig("checkpoints/qualcom_checkpoints/qvalidationloss"+ data_ix +"_conv_speech_loss.png")
